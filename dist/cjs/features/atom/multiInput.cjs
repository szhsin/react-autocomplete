...
//#region src/features/atom/multiInput.ts
const multiInput = () => ({ selected: _selected, setFocusIndex, removeSelect, isEqual }) => {
	...
	const [activeTag, setActiveTag] = (0, react.useState)(-1);
	...
	return {
		...
		getInputProps: () => ({
			...
			onKeyDown: (e) => {
				...
				switch (e.key) {
					case "Backspace": removeSelect(selected[activeTag]);
					case "ArrowUp":
					case "ArrowDown":
					case "Escape":
						resetTag();
						break;
					case "ArrowLeft":
						if (nextTag < 0) nextTag = tagLength - 1;
						else if (nextTag > 0) nextTag -= 1;
